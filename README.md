...
- A **Privacy policy** link sits below the categories, opening the hosted policy in a browser with an `ACTION_VIEW` intent, and falling back to a Toast naming the address if the device has nothing that can open it. Behind the gate deliberately: Play's Families policy wants the policy reachable from inside the app, and equally does not want a child one tap from the open web. The URL points at the GitHub Pages address `docs/privacy-policy.html` will have, which does not resolve until the repository is public and Pages is enabled — see [Releasing](#releasing). Above Attribution because it is an action and Attribution is not; below the categories because it is not what a parent came in here to do.
...
- `PRIVACY_POLICY_URL` in `SettingsScreen.kt` is `https://ivancesar.github.io/kid-android-image-app/privacy-policy.html`, the address `docs/privacy-policy.html` will have once the repository is public and GitHub Pages serves `/docs` from `main`. Until both are done the link 404s from inside Parent Settings. The same address goes in the Play Console listing — Play checks that it resolves. `docs/play-store-submission.md` has the steps.
