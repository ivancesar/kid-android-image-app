...
- A **Privacy policy** link sits below the categories, opening the hosted policy in a browser with an `ACTION_VIEW` intent, and falling back to a Toast naming the address if the device has nothing that can open it. Behind the gate deliberately: Play's Families policy wants the policy reachable from inside the app, and equally does not want a child one tap from the open web. The URL is still the `example.com` placeholder in `SettingsScreen.kt` — see [Releasing](#releasing). Above Attribution because it is an action and Attribution is not; below the categories because it is not what a parent came in here to do.
...
- `PRIVACY_POLICY_URL` in `SettingsScreen.kt` is still `https://example.com/kids-explore/privacy-policy`. It has to be replaced with the real hosted address of `docs/privacy-policy.html` before upload, and the same address goes in the Play Console listing — Play checks that it resolves.
