...
The practical cost is that nothing in the semantics tree says *which* photograph is on screen, so the image carries `VIEWER_IMAGE_TEST_TAG` for the tests to find it, and they assert on the ViewModel's index rather than on anything rendered.
